:lang: ja
:doctype: book
:toc: left
:toclevels: 3
:toc-title: EnergyPlus SHASE-G1008:2016
:sectnums:
:sectnumlevels: 4
:sectlinks:
:example-caption: 例
:table-caption: 表
:figure-caption: 図
:docname: EnergyPlus SHASE-G1008:2016
:stem: latexmath
:xrefstyle: short
:icons: font

= SHASE-G 1008:2016 + EnergyPlus
建物エネルギーシミュレーションツールの評価手法に関するガイドラインの適用事例（EnergyPlus Ver.9.4）

== はじめに

SHASE-G1008:2016「建物エネルギーシミュレーションツールの評価手法に関するガイドライン」をEnergyPlusに対して適用する方法を解説します。 +
EnergyPlusはVer.9.4を使用しています。EnergyPlusのインストール方法や使用方法については省略します。


NOTE: SHASE-G1008:2016は http://www.shasej.org/tosho/report.html[空気調和・衛生工学会] にて購入できます。

TIP: EnergyPlusの入力ファイル（IDFファイル）の作成方法の詳細は https://bigladdersoftware.com/epx/docs/9-4/input-output-reference/index.html[bigladder, role="external", window="_blank"] を参照してください。


<<<
== 熱負荷シミュレーション

SHASE-G1008:2016 第5章「熱負荷シミュレーションツールのテスト」を実行します。

=== Case600

include::case600.adoc[]

<<<
=== Case610

include::case610.adoc[]

<<<
=== Case650

include::case650.adoc[]

<<<
=== Case900

include::case900.adoc[]


<<<
=== Output:Variable

==== Surface Outside Face Incident Solar Radiation Rate per Area

https://bigladdersoftware.com/epx/docs/9-4/input-output-reference/group-thermal-zone-description-geometry.html#surface-outside-face-incident-solar-radiation-rate-per-area-wm2[<bigladder>, role="external", window="_blank"]

<<<
=== 地盤境界条件の入力方法

include::groundTemperature.adoc[]

