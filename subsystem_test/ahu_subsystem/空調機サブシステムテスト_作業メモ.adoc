...
* "WarmestTemperatureFlow"を用いる場合、Case 200（負荷均一）では他のツールと近い結果になった。Case 300（負荷偏在）では、なぜか給気温度が下限値の16℃となり、負荷率の低いゾーンNPでは室温設定を満たせず過剰冷房となった。
* エネルギーの観点で見ると、冷水コイル処理熱量はどちらの場合でも他のツールとほぼ同等の値となっている。一方、ファン電力については両制御でCase 200, 300で逆の傾向になっている。

==== 今後の課題
* EnergyPlusの給気温度リセット制御についてもう少し詳細な勉強、調査が必要。"WarmestTemperatureFlow"は負荷偏在時に室温設定値を満たせないとすると致命的な欠陥があると言える。また、暖房時は"WarmestTemperatureFlow"と対になる制御がない。そう考えると"Warmest"が無難かもしれない。
